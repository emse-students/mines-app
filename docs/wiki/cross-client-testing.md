...
| HEAL-REVOKE-4 | The heal-on-diff mechanism catches up what the first reconnection missed, and the TRIGGER CONDITIONS are part of the assertion | `W1 W2` | **`FAIL`** 2026-09-06 on `c643a411`, **clean**, `unmet: ['theREFERENCEGotWhatWasSaid', 'theAskerAPPLIEDTheAnswer']` - **the row refuses to conclude, which is what it was written to do**: the CONTROL failed too (reference 0 of 3, `equalityGap: []`), so an equality between two devices that both got nothing is not evidence about the subject. **Its trigger clauses all held** - `escalatedPastASilentResponder: true`, `refusedBecauseItAlreadyAsked: true`, so the anti-fake assertion (the mechanism caught DECLINING to act) is satisfied and the row is not vacuous. **What it found is a P1 that is not its own subject**: the `history_digest_request` that repairs a rejoining device is received **314 s** after it is sent, reproduced at 308 s and 300 s across two devices and two builds, with the device provably idle in between. **Two suspects were excluded by measurement, not argument** - the ack barrier and the exclusion-reason bug, both fixed the same day, neither moved it ([backlog](backlog.md)). The row is owed a re-run once that P1 is answered |
