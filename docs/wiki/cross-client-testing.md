...
| HEAL-repair | Does the history diff repair a rewound sender end to end? Quantitative | `+snapshot` | **`PARTIAL`** 2026-09-05, first run - **7 of 14 reached the peer**, and the seven that failed are the first seven: nothing arrived until round 8, then every round did. The mechanisms say which repair fired and it is not the one the row is named for: `lossDetected=true`, `historyDiffRan=true`, but `narrowRetransmission=false` and `escalated=false`, and the diff asked three times without being answered - `[HISTORY_REQ] ... asked ... to describe itself, no digest came`. **THAT STRING IS ANSWERED AND THE LINE NO LONGER EXISTS, 2026-09-05/06.** It was not the responder failing to ask: it was the ASKER answering after its own mailbox had drained while the responder waited `DIGEST_TTL_MS` = 60 s - measured at **67 s against that 60 s wait**, the digest arriving seven seconds after it gave up. A digest carries the manifest and the window, so answering one needs nothing remembered: the second leg does not wait at all now, and the frame is answered wherever it lands. **This row is owed a re-run on that build**, and the P1 in [backlog](backlog.md) carries the trail plus the two further defects the same measurement uncovered - a retry the server's random election assumes and the client did not make, and an exclusion filter that had never accepted a real member key. So the group came back without the mechanism that is supposed to bring it back. Teardown clean: W1 delivered again after 1 probe. [backlog](backlog.md) |
...
| HEAL-REVOKE-5 | Revoked, then the account CHANGES a lot while it is away, then it returns | `W1 W2` `+user` | **`FAIL`** 2026-09-05 21:51 on `2862d958`, clean, `unmet: ['theSameMESSAGESArrived']` - **and the STRUCTURAL half passed**: `equalityGap: []`, the returned device ending on exactly the group readiness a device minted 90 s later reaches. **What fails is CONTENT.** Three messages were said in a group while the device was revoked; the returned device saw **0 of 3 after waiting 60 s** and the reference **3 of 3 in 2 ms**. Five reproductions, same numbers. Not the instrument (equal budgets, and a RELOAD still shows 0, so they are not in its store), not forward secrecy (that blinds both, which is why the assertion is an EQUALITY). Both devices take the same path verbatim - external join, self-service - and send the same state key. **The cause is a race between two clocks on two devices**: W1 answers `Keys differ - describe yourself` and waits 60 s (`DIGEST_TTL_MS`); the returning device answers only once its own mailbox has drained, and it is applying twenty external joins, so its digest lands **67 s** later - seven seconds after W1 logged `no digest came`. **FIXED, and this row is owed a re-run.** Nothing retried, and HEAL-REVOKE-7's order pair said why: the retry trigger DOES fire (`holds 4 frame(s) it can never read`, six seconds after the join) and was swallowed by the 30 s coalescing window - the run whose first ask nobody could answer was the one that ended up COMPLETE. Three defects came out of that thread and all three are fixed: the second leg needed a live waiter, the client never made the retry the server's random election is built around, and the exclusion that retry needs was dropped by a 128-character filter against 147-character keys. On the fixed build HEAL-REVOKE-7 reads **3 of 3 in ~1 s** where it read 0 of 3 for ever. P1 in [backlog](backlog.md) with the trail. **The row had NEVER measured content until 2026-09-05**: its own docstring claimed messages were sent while the device was away and the code moved membership only |
...
| HEAL-REVOKE-7 | The **ORDER** of the return: back BEFORE the other devices are online, and back AFTER | `W1 W2` `+user` | **`FAIL` for the PAIR**, and the row found THREE defects and drove all three to a fix (2026-09-05/06, nine runs). **The LOSS is gone**: `--order last` was `0 of 3 after 60 s, for ever` in eight consecutive runs and is now `3 of 3 in ~1 s`, measured on both devices with healthy panes (`composer: true`, the right header, `bodyCount: 4`). What it cost: the responder's second leg needed a live waiter and gave up seven seconds before a busy peer answered; the server elects at RANDOM so a retry can rotate past a frozen phone and there was no retry; and the exclusion that retry needs was filtered by `k.length <= 128` against member keys of **147 and 149** characters, so it had never excluded anybody and neither had the coverage chase. **What remains is a DURATION, not a loss** - the repair waits on the asking device's WHOLE mailbox, and a device rejoining 29 groups took **189 s** once and over 300 s once, so whichever side is slower that run misses the budget and the pair flaps. P2 with the per-group barrier that removes it in [backlog](backlog.md). The row's own instrument was corrected three times on the way: it now records the `SAMPLE` discriminators on every reading, refuses to call an equality between two failures a pass (`theREFERENCEGotWhatWasSaid`), and gives the repair 300 s while recording what it really took |
| HEAL-REVOKE-8 | A group **DELETED while the device was revoked** must not return as a Sync row | `W1 W2` `+user` | **`FAIL`** 2026-09-05 22:09 on `2862d958`, `unmet: ['theSameMESSAGESArrived']` - **and the row's OWN subject passed**: the deleted group is absent from the returned device AND from a reference minted minutes later, 21 of 21 rows ready on both, `equalityGap: []`. It is not vacuous - `deletionSucceeded: true`, and the row asserts that separately for exactly this reason. **The single failing tick is the P1 the rung found on HEAL-REVOKE-5**, reproduced here on a different row with the same numbers: the returned device saw **0 of 3 after 60 s**, the reference **3 of 3 in 3 ms** ([backlog](backlog.md)). Dirt is one line, the filed `[QUEUE] delivery ... arrived twice` |
