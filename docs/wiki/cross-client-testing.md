...
| NOTIF-10 | Airplane mode 10 min, 5 messages, then reconnect: all five survive | `+push` | **`FAIL`** 2026-09-05 09:40 on `768a6c1b` + APK `93856118` - **first run ever, and the one row where the shared dirt becomes the verdict.** The shade holds `Nouveau message de ...` with no preview and `notifiedInMs: null`: no decrypted notification ever arrived for any of the five. The radios are CUT here rather than the app killed, so when they return the app's own WebSocket and the FCM push handler process the same messages and the push loses - `Ciphertext generation out of bounds 433/434/435`, three consumed generations, no push ever succeeding. **The client-side half of that was fixed 2026-09-05** (a backgrounded phone now notifies for the frame IT received, which is the one the push will never carry); this row is owed a re-run, and it is the one row where the shared dirt was ever a verdict. P1 in [backlog](backlog.md) |
...
| HEAL-REVOKE-2 | The revoked device reconnects: is it like-new, holding nothing from before? The blacklist can make this row pass while HEAL-REVOKE-1 fails | `W1 W2` | **`PASS-DIRTY`** 2026-09-05 21:20 on `2862d958`, `unmet: []`, **and it is written so the blacklist cannot satisfy it**. A revoked id is refused, so "it came back as a new device" is true of a device that kept every byte - the row therefore leans on a group DELETED while it was away, which the server will never serve and enumeration cannot produce: a row for it can come from a surviving store and nowhere else. Absent. Its mirror held too - the group CREATED while it was away arrived ready, so the absence is a device that FORGOT rather than one that never arrived. 12 of 12 rows ready in 4.0 s, `amber: []`, a conversation opened in 16 ms while it healed. **It does NOT assert that the first readout shows zero ready rows** - the sharpest evidence there is, and unreadable here: the seed settled in 1 ms. **The `DIRTY` is one line, the same filed `[QUEUE] delivery ... arrived twice`** that dirties TAB-3b, HEAL-REVOKE-9 and HEAL-REVOKE-3 - deliberately not forgiven. One disposition was written on the way: a device that has just erased itself asking for re-admission to a group it must serve an invitation for, which `actions.ts` names as the recovery seam ([backlog](backlog.md)) |
| HEAL-REVOKE-3 | First reconnection after revocation resynchronises as a NEW device would, history included - a shortfall must be REPORTED, not silently partial | `W1 W2` | **`PASS-DIRTY`** 2026-09-05 21:18 on `2862d958`, `unmet: []`, **`equalityGap: []`**. The returned device and a device minted MINUTES LATER on the same profile in the same world end identical: 11 rows, 11 ready, `amber: []`, both. **The reference is measured, never looked up** - two fingerprints from the same quarter-hour are a comparison, one fingerprint and a remembered number is a hope. It also carries the assertion no other row makes, and it is CONDITIONAL by construction: **a shortfall must be VISIBLE**, so if the returned device holds less than the reference the app must still be saying so with rows amber. The user's injury was never a device that ended short, it was one that ended short and LOOKED COMPLETE. On a run with no shortfall it claims nothing, which is honest rather than weak. Time is dirt carrying a number and not a failure: 4.0 s against the reference's 1 ms, because the returning device pays a login the reference has already paid. **The `DIRTY` is one line**, the filed `[QUEUE] delivery ... arrived twice` - FOURTH sighting, still not forgiven |
| HEAL-REVOKE-4 | The heal-on-diff mechanism catches up what the first reconnection missed, and the TRIGGER CONDITIONS are part of the assertion | `W1 W2` | **`FAIL`** 2026-09-06 on `60432d09`, **clean**, `unmet: ['theREFERENCEGotWhatWasSaid', 'theAskerAPPLIEDTheAnswer']` - **the row refuses to conclude, which is what it was written to do**: the CONTROL failed too (reference 0 of 3, `equalityGap: []`), so an equality between two devices that both got nothing is not evidence about the subject. **Its trigger clauses all held** (`escalatedPastASilentResponder`, `refusedBecauseItAlreadyAsked`), so the anti-fake assertion - the mechanism caught DECLINING to act - is satisfied and the row is not vacuous. **What it found is a P1 that is not its own subject**: `history_digest_request` is never ROUTED to the rejoining device at all - the gateway shows one frame to it at join and nothing after - so the exchange waits for the next connection edge. **The '300 s' in the first reading was this row's own budget**: `messagesIn` reloads on giving up, and the digest goes out 4 s and 14 s later. **Two suspects were excluded by measurement, not argument** - the ack barrier and the exclusion-reason bug, both fixed the same day, neither moved it, which is what forced the reading onto the GATEWAY ([backlog](backlog.md)). Owed a re-run once that P1 is answered |
| HEAL-REVOKE-5 | Revoked, the world changes A LOT, then it returns - the equality has to hold over MANY groups and both kinds of change | `W1 W2` `+user` | **`PASS-DIRTY`** 2026-09-06 01:04 on `4d184676`, `unmet: []`, `equalityGap: []` - **the P1 it found is closed on the row that found it**: both devices see 3 of 3 in ~1 s where the returned device saw 0 of 3 after 60 s, six runs running. Four defects stood behind that one reading and HEAL-REVOKE-7 carries the list. **The single dirt line is a sighting of an OPEN item, not a new one**: `[KICK] Stale leaf <the phone> removed from ba048e26…` is W1 answering a `welcome_request` for a group the phone already has a leaf in - the documented kick-and-re-add - and the phone asking at all is the P1 in [backlog](backlog.md) about a device that asks for a Welcome for ever. **This row is `PASS` the day that one is** |
...
| HEAL-REVOKE-8 | A group **DELETED while the device was revoked** must not return as a Sync row | `W1 W2` `+user` | **`PASS-DIRTY`** 2026-09-06 01:15 on `4d184676`, **`unmet: []`** and `equalityGap: []` - **it was `FAIL` on `2862d958` for exactly one tick, `theSameMESSAGESArrived`**, and that tick was the P1 this rung found on HEAL-REVOKE-5. Returned device **3 of 3 in 501 ms**, reference 3 of 3 in 505 ms, and the deleted group is absent from both. **The trail names every repaired mechanism in order and is the best evidence the campaign holds for the fix**: the first ask at `+0s` goes unanswered, the coalescer refuses a second at `+9s` (`was asked 30s ago at most`), then `holds 4 frame(s) it can never read` fires the escalation - `still holds frames it cannot read while <the phone> answers nothing - electing somebody else` - and `3 messages received from the inviting peer` lands in the SAME SECOND. Dirt is one line and it is the open Welcome-for-ever P1; **it names the same phone that then answers nothing**, which is a correlation, not a coincidence ([backlog](backlog.md)) |
| HEAL-REVOKE-9 | Revoked while **OFFLINE**: the wipe is DEFERRED, not lost, and must not fire while the server is unreachable | `W1 W2` | **`PASS-DIRTY`** 2026-09-05 20:43, **first run ever, and all three assertions held**: while the device was unreachable `wipeRan: false` and the residue was intact; after a bare RELOAD, still `wipeRan: false` and still holding, sitting at `/login` - a destructive control refusing to fire on a state nothing had confirmed; and the wipe landed on the sign-in. **The `DIRTY` is one line and it is a real, filed defect**: `[QUEUE] delivery ... arrived twice`, the same duplicate that dirties TAB-3b. It is deliberately NOT forgiven - a row made clean by widening a needle is worse than one reporting honestly. Two dispositions were written on the way, both naming FOREIGN noise this rig had been reading as the app's: the IdP's own console and a revoked device's account of itself ([backlog](backlog.md)) |
