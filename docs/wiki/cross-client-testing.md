...
| HEAL-repair | Does the history diff repair a rewound sender end to end? Quantitative | `+snapshot` | **`PARTIAL`** 2026-09-05, first run - **7 of 14 reached the peer**, and the seven that failed are the first seven: nothing arrived until round 8, then every round did. The mechanisms say which repair fired and it is not the one the row is named for: `lossDetected=true`, `historyDiffRan=true`, but `narrowRetransmission=false` and `escalated=false`, and the diff asked three times without being answered - `[HISTORY_REQ] ... asked ... to describe itself, no digest came`. **THAT STRING IS ANSWERED, 2026-09-05 22:00, by HEAL-REVOKE-5.** It is not the responder failing to ask: it is the ASKER answering after its own mailbox has drained while the responder waits only `DIGEST_TTL_MS` = 60 s, so a device with a busy queue misses the window - measured at **67 s against a 60 s wait**, the digest arriving seven seconds after the responder gave up. One cause, two rows; the P1 in [backlog](backlog.md) carries the second-by-second trail and what a fix has to satisfy. So the group came back without the mechanism that is supposed to bring it back. Teardown clean: W1 delivered again after 1 probe. [backlog](backlog.md) |
...
| HEAL-REVOKE-7 | The **ORDER** of the return: back BEFORE the other devices are online, and back AFTER | `W1 W2` `+user` | **`FAIL` for the PAIR**, 2026-09-05 22:15 on `2862d958`. `--order first` is `PASS-DIRTY` (`unmet: []`, 23 of 23 ready, the three messages in **3.5 s**); `--order last` is `FAIL` (`unmet: ['theSameMESSAGESArrived']`, 22 of 22 ready, `equalityGap: []`, **0 of 3 after 60 s and still 0 after a reload**). The pair is adjudicated on FINAL STATE and the two differ. **Same build, same runner, one variable - so the pair IS the controlled experiment**, and it dates the P1's cause to the second: the returning device asks once at the join, and the ONE retry it has (`holds N frame(s) it can never read`) is swallowed by `PROBE_COALESCE_MS` = 30 s when it fires **6 s** after that ask (`last`), while it produces a real second ask when it fires **42 s** after it (`first`, where nobody had been online to answer the first). That second ask is the one that works - the mailbox has drained, the digest goes out in 2 s, `history_bundle` lands, 3 of 3. Dirt is one line on the reference, the filed `[QUEUE] delivery ... arrived twice` ([backlog](backlog.md)) |
